...
def draw_bar(points: list[Point], imp_th: float, sat_th: float, output: Path, dpi: int = 320) -> None:
    ...
    ax.axhline(imp_th, color="#1d4ed8", linestyle="--", linewidth=1.0, label=f"重要度均值线 {imp_th:.4f}")
    ax.axhline(sat_th, color="#b45309", linestyle="--", linewidth=1.0, label=f"满意度均值线 {sat_th:.4f}")
