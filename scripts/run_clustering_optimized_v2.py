...
OUT_DIR = ROOT / "data" / "data_clustering1"
...
def external_validity_rows_stratified(data, labels, feature_set, preprocess, k):
    ...
    mean_effect = float(np.nanmean(np.array(all_effects, dtype=float))) if all_effects else np.nan
    ...
    summary = {
        ...
        "visited_sig_n": int(next((r["sig_vars_n"] for r in summary_rows if r["layer_id"] == "visited"), 0)),
        "unvisited_sig_n": int(next((r["sig_vars_n"] for r in summary_rows if r["layer_id"] == "unvisited"), 0)),
        "all_sig_n": int(next((r["sig_vars_n"] for r in summary_rows if r["layer_id"] == "all"), 0)),
    }
...
def choose_model(model_rows):
    ...
    for r in model_rows:
        if r["feature_set"] == FINAL_FEATURE_SET and r["preprocess"] == FINAL_PREPROCESS and r["k"] == FINAL_K_MAIN:
            main_model = r
        if r["feature_set"] == FINAL_FEATURE_SET and r["preprocess"] == FINAL_PREPROCESS and r["k"] == FINAL_K_APPENDIX:
            appendix_model = r
    if main_model is None:
        raise RuntimeError("cannot find configured main model")
    if appendix_model is None:
        appendix_model = main_model
    reason = "按改进方案2收口：正文主口径固定为 enhanced + robust + K=2；K=4 作为统计优先附录对照。"
    return main_model, appendix_model, reason
...
def main():
    ...
    write_dict_csv(
        ...
        ["feature_set", "preprocess", "k", "layer_id", "layer_name", "layer_n", "tested_vars_n", "sig_vars_n", "mean_effect", "layer_score"],
        external_summary_all,
    )

    selected, appendix_model, selection_reason = choose_model(model_rows)
    ...
    # Appendix profile for K=4 under final feature/preprocess.
    appendix_state = model_states[(appendix_model["feature_set"], appendix_model["preprocess"], appendix_model["k"])]
    appendix_profiles = cluster_profiles(data, appendix_state["labels"])
    write_dict_csv(
        OUT_DIR / "聚类优化_画像卡_K4附录.csv",
        [
            "cluster",
            "cluster_name",
            "n",
    ...
    for r in k_robust_rows:
        ...
        elif r["k"] == appendix_model["k"]:
            r["k_robust_note"] = "附录统计优先对照"
        else:
            r["k_robust_note"] = "备选"
    write_dict_csv(
    ...
    lines.append(
        f"- 附录对照: feature_set={appendix_model['feature_set']} | preprocess={appendix_model['preprocess']} | "
        f"K={appendix_model['k']} | silhouette={appendix_model['silhouette']:.4f} | stratified_score={appendix_model['stratified_external_score']:.4f}"
    )
    ...
    lines.append(
        ...
        f"到访层显著数={selected['visited_sig_n']}；未到访层显著数={selected['unvisited_sig_n']}"
    )
    ...
    write_rows_csv(
        ...
        [
            ...
            ["appendix_k", appendix_model["k"]],
            ["appendix_silhouette", appendix_model["silhouette"]],
            ["appendix_stratified_external_score", appendix_model["stratified_external_score"]],
