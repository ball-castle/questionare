...
二、正式调查口径
1) 本轮问卷分析以原始文件《原始数据_Amethyst.xlsx》有效样本813为统一口径。
2) 关键模型与表格均基于同一口径输出，保证可追溯与复现。

三、质量控制说明
1) 注意力题采用“双口径并行”（应选1与应选5）做稳健性对比。
2) 主分析保留平衡口径，不做大规模硬剔除；敏感性分析用于说明结论稳定性边界。
"""
    (OUT_TEXT / "国奖补强_口径声明.txt").write_text(txt, encoding="utf-8")


def make_pretest_formal_diff():
    rows = [
        {
            "对比维度": "研究目的",
...
def main():
    ...
    make_pretest_statement()
    make_pretest_formal_diff()
